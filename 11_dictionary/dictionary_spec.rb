...
# [Matchers] ( https://relishapp.com/rspec/rspec-expectations/v/3-5/docs/built-
# in-matchers/be-matchers )
...
describe Dictionary do
  ...
  it "doesn't cheat when checking whether a given keyword exists" do
    expect(@d.include?('fish')).to be_falsey # if the method is empty, this test
     # passes with nil returned
    @d.add('fish')
    expect(@d.include?('fish')).to be # confirms that it actually checks
    expect(@d.include?('bird')).to be_falsey # confirms not always returning
    # true after add
  end
  ...
  it 'can produce printable output like so: [keyword] "definition"' do
    ...
    @d.add('fish' => 'aquatic animal')
    @d.add('apple' => 'fruit')
    expect(@d.printable).to eql(%([apple] "fruit"\n[fish] "aquatic animal"\n[zebra] "African land animal with stripes"))
  end
end
